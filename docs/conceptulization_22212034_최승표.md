...
| 3 | 예약 관리 (CRUD) | Admin | 식별자(학번/사번)를 기반으로 신규 예약을 등록하고, 시간 충돌 및 자원 상태를 검증하며 기존 내역을 수정하거나 삭제한다. |
...
| | |
| :--- | :--- |
| **Purpose** | 식별자(학번/사번)를 기반으로 예약을 등록하고 시스템 규칙을 자동 검증한다. |
| **Approach** | 캘린더 인터페이스에서 원하는 날짜/시간을 선택하고 식별자를 입력하면, 시스템이 시간 충돌, 자원 가용 상태, 제재 여부를 자동으로 확인하여 예약을 확정한다. |
| **Dynamics** | 이용자가 특정 자원에 대해 신규 예약을 요청하거나 기존 예약을 수정할 경우 |
| **Goals** | 3중 자동 검증(시간, 상태, 권한)을 통해 휴먼 에러와 이중 예약을 원천 차단한다. |
...
4. **예약 (Reservation)**  특정 사용자(식별자)와 자원을 특정 시간대에 연결하는 핵심 트랜잭션. 
